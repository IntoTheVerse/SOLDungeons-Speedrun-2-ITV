...
public class Chest : MonoBehaviour, IUseable
{
    ...
    /// </summary>
    public void Initialize(bool shouldMaterialize, int healthPercent, WeaponDetailsSO weaponDetails, int ammoPercent, bool getToken)
    {
        this.healthPercent = healthPercent;
        this.weaponDetails = weaponDetails;
        this.ammoPercent = ammoPercent;

        if ((WalletManager.instance.playerLevel < GameManager.Instance.currentDungeonLevelListIndex + 1) && getToken)
            tokenAmount++;
    ...
    private void UpdateChestState()
    {
        ...
        {
            chestState = ChestState.tokenItem;
            InstantiateTokenItem();
        }
        else
        {
    ...
    /// <summary>
    /// Instantiate token item for the player to collect
    /// </summary>
    private void InstantiateTokenItem()
    {
        InstantiateItem();

        chestItemGameObject.GetComponent<ChestItem>().Initialize(GameResources.Instance.tokenIcon, "DUN Token", itemSpawnPoint.position, materializeColor);
    }

    /// <summary>
    /// Collect token and add it to the token mint
    /// </summary>
    private void CollectTokenItem()
    {
        // Check item exists and has been materialized
        if (chestItem == null || !chestItem.isItemMaterialized) return;
