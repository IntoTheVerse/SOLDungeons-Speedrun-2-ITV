...
public class PlayerControl : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        movementJoystick = manager.movementJoystick;
        aimJoystick = manager.aimJoystick;
        manager.ShootButton.onClick.AddListener(() => Fired());
        manager.ReloadButton.onClick.AddListener(() => {
            reload = true;
            ReloadWeaponInput();
        });
        manager.prevGun.onClick.AddListener(() => PreviousWeapon());
        manager.nextGun.onClick.AddListener(() => NextWeapon());
#else
    ...
    private void MovementInput()
    {
        // Get movement input
#if UNITY_IOS || UNITY_ANDROID
        float horizontalMovement = movementJoystick.Direction.x * 2;
        float verticalMovement = movementJoystick.Direction.y * 2;
#else
    ...
    private void AimWeaponInput(out Vector3 weaponDirection, out float weaponAngleDegrees, out float playerAngleDegrees, out AimDirection playerAimDirection)
    {
        ...
        Vector3 mouseWorldPosition = HelperUtilities.GetMouseWorldPosition(mousePos);
#else
